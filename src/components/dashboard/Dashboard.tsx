...
const Dashboard: React.FC = () => {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://${hostname}:3000/api/v1/search/stats`);
  ...
  if (error) return (
    <div className="p-8 text-white font-mono bg-black">
      <div className="border border-white/20 p-6 flex flex-col items-center gap-4">
        ...
        <button onClick={fetchStats} className="px-4 py-2 border border-white text-xs hover:bg-white hover:text-black transition-all">RETRY.SYSTEM()</button>
      </div>
    </div>
  );
  ...
  const interfaceData = [
    ...
    { name: 'OTHER', value: stats.interfaceStatus?.other || 0, color: '#111111' },
  ];
  ...
  const StatCard = ({ title, value, icon: Icon, subtext }: any) => (
    <div className="bg-neutral-900/40 border border-white/10 p-5 space-y-3 hover:border-white/30 transition-all group">
      <div className="flex justify-between items-start">
        ...
        <div className="text-[8px] font-bold text-neutral-600 uppercase tracking-widest">Live.Feed</div>
      </div>
      <div>
        <div className="text-2xl font-bold text-white tracking-tighter">{value}</div>
        <div className="text-[9px] text-neutral-500 uppercase font-bold tracking-[0.2em]">{title}</div>
      </div>
      {subtext && <div className="text-[8px] text-neutral-600 border-t border-white/5 pt-2">{subtext}</div>}
    </div>
  );
      ...
      <div className="flex justify-between items-end border-b border-white/10 pb-6">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold tracking-tighter uppercase">Network.Intelligence.Center</h1>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-neutral-900/20 border border-white/10 p-6 space-y-6">
          <div className="flex justify-between items-center border-b border-white/10 pb-4">
            ...
            <span className="text-[8px] text-neutral-600 tracking-widest">X-AXIS: SUBNET_CIDR / Y-AXIS: DEVICE_COUNT</span>
          </div>
        ...
        <div className="bg-neutral-900/20 border border-white/10 p-6 space-y-6">
          ...
          <div className="h-[250px] relative">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-2xl font-bold tracking-tighter">{stats.interfaceStatus?.up}</span>
        ...
        <div className="bg-neutral-900/10 border border-white/10 p-6 space-y-4">
          ...
          <div className="space-y-2">
            {stats.topHubs?.map((hub: any, i: number) => (
              <div key={hub.name} className="group flex items-center justify-between p-3 bg-white/5 border border-transparent hover:border-white/20 transition-all">
                <div className="flex items-center gap-4">
                  <span className="text-neutral-600 text-[9px] font-bold">0{i+1}</span>
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-white group-hover:tracking-wider transition-all uppercase">{hub.name}</span>
                ...
                <div className="text-right">
                  ...
                  <div className="text-[8px] text-neutral-600 uppercase">Edges</div>
                </div>
              </div>
            ))}
          </div>
            ...
            {snmpData.map((item: any) => (
              <div key={item.version} className="space-y-1.5">
                <div className="flex justify-between items-center text-[9px] uppercase tracking-widest">
                  <span className="text-neutral-500">SNMP.Version_{item.version}</span>
                  <span className="text-white font-bold">{item.deviceCount} Units</span>
                </div>
                <div className="w-full h-1 bg-neutral-900">
                  <div 
                    className="h-full bg-white transition-all duration-1000" 
                    style={{ width: `${(item.deviceCount / stats.devices?.totalManaged) * 100}%` }}
                  />
                </div>
              </div>
            ))}
