        ...
        <section className="lg:col-span-2 bg-neutral-900/20 border border-white/10 p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div key={i} className="p-4 bg-neutral-900/40 border border-white/5 hover:border-white/20 transition-all group">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    <div className={`w-2 h-2 rounded-full shadow-[0_0_8px_rgba(255,255,255,0.4)] ${iface.latestData?.ifOperStatus === 1 ? 'bg-white' : 'bg-neutral-800 border border-white/20'}`} />
                    <span className="text-[8px] text-neutral-600 uppercase font-bold">{iface.latestData?.ifOperStatus === 1 ? 'Active' : 'Down'}</span>
                  </div>
                </div>
            ...
            {filteredInterfaces.length === 0 && (
              <div className="col-span-full py-12 text-center border border-dashed border-white/10">
                <span className="text-[10px] text-neutral-600 uppercase tracking-widest">No interfaces match the selected filter</span>
              </div>
            )}
          </div>
          ...
          {filteredInterfaces.length > 10 && (
            <div className="pt-4 flex justify-center">
              ...
              >
                {showAllInterfaces ? "Collapse.Matrix()" : `Expand.Matrix(${filteredInterfaces.length - 10}.More)`}
              </button>
            </div>
          )}
        </section>
