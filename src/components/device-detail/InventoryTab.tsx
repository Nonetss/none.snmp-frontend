      ...
      <section className="bg-neutral-900/20 border border-white/10 p-6 space-y-6">
        ...
        <div className="space-y-3 max-h-[700px] overflow-y-auto custom-scrollbar pr-2">
          {device.physicalEntities?.map((entity: any, i: number) => (
            <div key={i} className="p-4 bg-white/5 border border-transparent hover:border-white/10 transition-all">
              <div className="flex justify-between items-center mb-1">
                <span className="text-xs font-bold text-white uppercase">{entity.name}</span>
                <span className="text-[8px] px-1.5 py-0.5 bg-white/10 text-neutral-400 font-bold uppercase tracking-tighter">CLASS_{entity.class}</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-[9px] pt-2 border-t border-white/5">
                <div className="flex flex-col">
                  ...
                  <span className="text-white uppercase tracking-tighter">{entity.mfgName} {entity.modelName}</span>
                </div>
          ...
          {(!device.physicalEntities || device.physicalEntities.length === 0) && (
            <div className="text-center py-20 text-[10px] text-neutral-700 uppercase tracking-[0.2em]">No physical inventory data available</div>
          )}
        </div>
      </section>
            ...
            <div key={i} className="p-5 bg-neutral-900/40 border border-white/5">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-[8px] text-neutral-500 uppercase tracking-widest">{res.type}</span>
                  </div>
                </div>
              ...
              {res.swInstalled?.length > 0 && (
                <div className="mt-4 pt-4 border-t border-white/5">
                  <span className="text-[8px] text-neutral-600 uppercase font-bold block mb-2">Attached SW Packages: {res.swInstalled.length}</span>
                </div>
              )}
            </div>
          ))}
          {(!device.resources || device.resources.length === 0) && (
            <div className="text-center py-20 text-[10px] text-neutral-700 uppercase tracking-[0.2em]">No hardware resources found</div>
          )}
        </div>
      </section>
    </div>
