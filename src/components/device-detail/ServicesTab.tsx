...
export const ServicesTab: React.FC<ServicesTabProps> = ({ device }) => {
  ...
  const filteredServices = useMemo(() => {
    ...
    return device.services.filter((svc) => {
      const matchesSearch = svc.hrSWRunName.toLowerCase().includes(searchTerm.toLowerCase()) || 
      ...
      <section className="bg-neutral-900/20 border border-white/10 p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6">
          {filteredServices.map((svc: Service, i: number) => (
            <div key={i} className="p-4 bg-white/5 border border-white/5 hover:border-white/20 transition-all group">
              <div className="flex justify-between items-start mb-4">
                <div className={`p-2 border transition-all ${svc.hrSWRunStatus === 1 ? 'bg-white/5 border-white/10 group-hover:bg-white group-hover:text-black' : 'bg-neutral-900 border-white/5 text-neutral-700'}`}>
                  <Shield className="w-4 h-4" />
                </div>
                <div className="text-right">
                  <span className={`text-[8px] px-1.5 py-0.5 font-bold uppercase ${svc.hrSWRunStatus === 1 ? 'bg-white/10 text-neutral-300' : 'bg-neutral-900 text-neutral-600 border border-white/5'}`}>
                    {svc.hrSWRunStatus === 1 ? 'Running' : 'Other'}
                  </span>
                </div>
              </div>
              <h4 className="text-xs font-bold text-white uppercase mb-1 truncate" title={svc.hrSWRunName}>{svc.hrSWRunName || 'Unnamed Service'}</h4>
              <p className="text-[9px] text-neutral-500 line-clamp-2 italic mb-4 h-6" title={svc.hrSWRunPath}>
                {svc.hrSWRunPath || 'No path specified.'}
              </p>
              <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/5">
                <div className="flex flex-col overflow-hidden">
                  <span className="text-[7px] text-neutral-600 uppercase font-bold">Parameters</span>
                  <span className="text-[9px] text-neutral-400 truncate" title={svc.hrSWRunParameters}>{svc.hrSWRunParameters || 'None'}</span>
                </div>
          ...
          {filteredServices.length === 0 && (
            <div className="col-span-full h-64 flex flex-col items-center justify-center border border-dashed border-white/10 text-neutral-600">
              ...
              <span className="text-[10px] uppercase tracking-[0.4em]">No.Matching.Services.Found</span>
              <button 
