...
export const DashboardTab: React.FC<DashboardTabProps> = ({ device, setActiveTab, hasNeighbors }) => (
  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {device.ipSnmp?.addrEntries?.slice(0, 5).map((addr: AddrEntry, i: number) => (
            <div key={i} className="p-3 border border-white/5 bg-neutral-900/40">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-[8px] text-neutral-600 font-bold uppercase tracking-tighter">IF: {addr.ipAdEntIfIndex}</span>
              </div>
            </div>
          ))}
          {device.ipSnmp?.addrEntries?.length > 5 && (
            <button onClick={() => setActiveTab("interfaces")} className="w-full py-2 text-[8px] text-neutral-500 uppercase hover:text-white transition-all">View.All.Addresses({device.ipSnmp.addrEntries.length})</button>
          )}
        </div>
      </section>
      ...
      <section className="lg:col-span-2 bg-neutral-900/10 border border-white/5 p-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                {device.neighbor_discovery?.outbound?.slice(0, 2)?.map((n: any, i: number) => (
                  <div key={`out-${i}`} className="p-3 border border-white/5 bg-white/5 flex flex-col gap-1">
                    ...
                    <span className="text-[9px] text-neutral-500 uppercase tracking-tighter">{n.portId} {'->'} {n.portDesc}</span>
                  </div>
                ))}
              </div>
              <div className="space-y-2">
                ...
                {device.neighbor_discovery?.inbound?.slice(0, 2)?.map((n: any, i: number) => (
                  <div key={`in-${i}`} className="p-3 border border-white/5 bg-white/5 flex flex-col gap-1">
                    ...
                    <span className="text-[9px] text-neutral-500 uppercase tracking-tighter">{n.address || "N/A"}</span>
                  </div>
                ))}
              </div>
            </>
        ...
        {hasNeighbors && (
          <button onClick={() => setActiveTab("discovery")} className="w-full mt-2 py-2 border border-white/5 text-[8px] text-neutral-500 uppercase hover:bg-white/5 transition-all">Open.Discovery.Matrix()</button>
        )}
      </section>
    </div>
  </div>
