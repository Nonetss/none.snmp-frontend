...
const SubnetManager: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const response = await axios.get(`${import.meta.env.PUBLIC_BACKEND_URL}/api/v1/snmp/subnet`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await axios.patch(`${import.meta.env.PUBLIC_BACKEND_URL}/api/v1/snmp/subnet/${editingId}`, {
          cidr: formData.cidr,
          name: formData.subnetName,
        })
      } else {
        await axios.post(`${import.meta.env.PUBLIC_BACKEND_URL}/api/v1/snmp/scan`, formData)
      }
      setShowForm(false)
      setEditingId(null)
  ...
  const handleTriggerScan = async (subnet: SubnetInfo) => {
    ...
    try {
      await axios.post(`${import.meta.env.PUBLIC_BACKEND_URL}/api/v1/snmp/scan`, {
        cidr: subnet.cidr,
        subnetName: subnet.name,
      })
