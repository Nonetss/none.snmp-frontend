...
export default function LoginHistoryCard() {
  const [data, setData] = React.useState<LoginHistoryData | null>(null)
    ...
    try {
      ...
      setError(
        e?.response?.data?.message ||
          e?.message ||
          'Error al obtener el historial de inicio de sesión'
      )
    } finally {
      setLoading(false)
    }
    ...
    <>
      ...
      >
        <div className="space-y-4">
          ...
          {data && data.history && data.history.length > 0 && (
            ...
            >
              Ver historial ({data.history.length})
            </button>
          )}
        </div>
      </InfoCard>
      ...
      {isModalOpen && data && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
          <div className="w-full max-w-[600px] bg-neutral-950 border border-white/20 shadow-2xl animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col max-h-[80vh]">
            <div className="flex justify-between items-center p-6 border-b border-white/10 bg-white/5">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-[10px] text-neutral-500 uppercase tracking-widest">
                  {data.computerName ? `Node: ${data.computerName}` : 'Full access history'}
                </p>
              </div>
              <button
