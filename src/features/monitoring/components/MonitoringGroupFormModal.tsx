...
interface Props {
  group?: MonitoringGroup | null // If present, we are editing
  onClose: () => void
  onSuccess: () => void
}
...
export const MonitoringGroupFormModal: React.FC<Props> = ({ group, onClose, onSuccess }) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (group) {
        await axios.patch(`/api/v0/monitor/group/${group.id}`, payload)
      } else {
        await axios.post(`/api/v0/monitor/group`, payload)
      }
      onSuccess()
      onClose()
    } catch (err: any) {
      alert(err.response?.data?.message || 'Failed to save monitoring group')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/95 backdrop-blur-sm p-4 overflow-hidden">
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-black uppercase tracking-[0.2em] text-white">
                {group ? 'Edit.Monitoring.Group' : 'Create.Monitoring.Group'}
              </h2>
        ...
        <div className="flex-1 flex overflow-hidden">
          {/* Left Panel: Info & Filters */}
          <div className="w-80 border-r border-white/10 bg-black/40 flex flex-col overflow-y-auto p-6 space-y-8">
            {/* Group Info Section */}
            <div className="space-y-4">
              <div className="flex items-center gap-2 text-neutral-400">
                <Server className="w-3.5 h-3.5" />
                <span className="text-[10px] font-black uppercase tracking-widest">Group_Info</span>
              </div>
        ...
        <div className="p-4 border-t border-white/10 bg-white/5 flex items-center justify-between">
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              {submitting ? 'EXECUTING...' : 'Confirm.Group.Settings'}
            </button>
          </div>
        </div>
      </form>
