      ...
      <div className="flex flex-col gap-6 border-b border-white/10 pb-6">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 border border-white/10 text-[10px] font-bold uppercase hover:bg-white hover:text-black transition-all"
          >
            <Download className="w-3.5 h-3.5" /> Export_Report
          </button>
        </div>

        {/* Filters Bar */}
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 bg-white/5 p-4 border border-white/5">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-neutral-500" />
            <input
              type="text"
              placeholder="SEARCH_DEVICE..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-black border border-white/10 pl-10 pr-4 py-2 text-[10px] font-bold uppercase focus:outline-none focus:border-white/40"
            />
          </div>

          <div className="flex items-center gap-2">
            <span className="text-[9px] text-neutral-600 font-black uppercase">Status:</span>
            <div className="flex flex-1 p-0.5 bg-black border border-white/10">
              {['all', 'up', 'down'].map((s) => (
                <button
                  key={s}
                  onClick={() => setStatusFilter(s as any)}
                  className={`flex-1 py-1.5 text-[8px] font-black uppercase transition-all ${
          ...
          <div className="flex items-center gap-2">
            <span className="text-[9px] text-neutral-600 font-black uppercase">Time_Range:</span>
            <select
              value={timeRange}
              onChange={(e) => setTimeRange(e.target.value as any)}
              className="flex-1 bg-black border border-white/10 p-2 text-[10px] font-bold uppercase focus:outline-none focus:border-white/40"
            >
          ...
          <button
            onClick={fetchData}
            className="flex items-center justify-center gap-2 border border-white/10 hover:bg-white/5 text-[10px] font-bold uppercase transition-all"
          >
          ...
          <table className="w-full text-left text-[10px] uppercase font-mono">
            <thead>
              <tr className="border-b border-white/5 text-neutral-500">
                ...
                <th className="p-4 font-black text-right">Response</th>
              </tr>
            </thead>
              ...
              {filteredData?.ports
                ...
                .map((log, i) => (
                  <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="p-4 text-white font-black">{log.name}</td>
                    <td className="p-4 text-neutral-500">{log.ipv4}</td>
                    <td className="p-4">
                      <span className="px-1 bg-white/5 border border-white/5 text-neutral-400">
                        :{log.port}
                      </span>
                    </td>
                    <td className="p-4">
                      <span
                        className={`px-2 py-0.5 font-black ${log.status ? 'text-emerald-500 bg-emerald-500/10' : 'text-red-500 bg-red-500/10'}`}
                      >
                        {log.status ? 'OK' : 'FAIL'}
                      </span>
                    </td>
                    <td className="p-4 text-right font-bold text-neutral-300">
                      {log.resp !== null ? `${log.resp}ms` : '---'}
                    </td>
                  </tr>
                ))}
            </tbody>
