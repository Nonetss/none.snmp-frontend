  ...
  return (
    <div className="space-y-16 animate-in fade-in duration-500 max-w-5xl mx-auto pb-20">
      ...
      <div className="space-y-6">
        ...
        <InfoCard
          title="Software.Binary_Query"
          description="Find nodes where a specific package is either present or missing from system inventory"
          icon={Box}
        >
          <div className="space-y-6">
            <form onSubmit={handleDirectSearch} className="flex flex-col md:flex-row gap-4">
              <div className="relative flex-1 group">
                ...
                <input
                  ...
                  placeholder="EXACT_APPLICATION_NAME"
              ...
              <div className="flex bg-black border border-white/10 p-1">
                <button
                  ...
                  className={`px-6 py-1.5 text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${isInstalled ? 'bg-white text-black' : 'text-neutral-500 hover:text-white'}`}
                >
      ...
      <div className="space-y-6">
        ...
        <InfoCard
          title="Smart.Fuzzy_Search"
          description="Pattern matching across all inventoried software using partial identifiers"
          icon={Zap}
          headerAction={
            fuzzyMeta &&
            fuzzyMeta.totalPages > 1 && (
              <div className="flex bg-black border border-white/10 p-1">
                <button
                  ...
                  className="p-1 hover:bg-white hover:text-black text-neutral-500 transition-all disabled:opacity-20"
                >
          ...
          <div className="space-y-8">
            <form onSubmit={(e) => handleFuzzySearch(e, 1)} className="flex gap-2">
              <div className="relative flex-1 group">
                ...
                <input
                  ...
                  placeholder="SEARCH_PARTIAL_NAME (e.g. Chrome, Office, SQL)"
            ...
            <div className="divide-y divide-white/5 bg-black border border-white/10">
              {fuzzyResults.map((app) => (
                <div key={app.name} className="group/item">
                  <button
                    onClick={() =>
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex flex-col items-start">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[9px] text-neutral-400 font-mono uppercase tracking-tighter">
                            Hits: {app.devices.length} Nodes
                          </span>
                          <span className="text-[8px] text-neutral-500 font-mono tracking-tighter">
                            UID_{getTechId(app.name)}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-6 bg-white/[0.01] border-t border-white/5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 animate-in slide-in-from-top-2">
                      {app.devices.map((device) => (
                        <a
                          key={device.id}
                          href={`/devices/${device.id}`}
                          className="p-4 border border-white/5 bg-black hover:border-white/20 transition-all group/node relative overflow-hidden"
                        >
      ...
      <div className="space-y-6">
        ...
        <InfoCard
          title="Software.Registry_Log"
          description="Master inventory of all unique software detected across the managed network"
          icon={Database}
          headerAction={
            <div className="flex items-center gap-4">
        ...
        >
          <div className="divide-y divide-white/5 bg-black border border-white/10 max-h-[800px] overflow-y-auto custom-scrollbar">
            {inventory.map((app) => (
              <div key={app.name} className="group/item border-b border-white/5 last:border-0">
                <button
                  onClick={() =>
                ...
                >
                  <div className="flex items-center gap-6">
                    <div className="w-10 h-10 border border-white/5 bg-white/[0.01] flex items-center justify-center group-hover:border-white/10 transition-colors">
                      <Box className="w-5 h-5 text-neutral-500 group-hover:text-white" />
                    </div>
                    ...
                    {app.devices.map((device) => (
                      ...
                      >
                        <div className="flex flex-col gap-1.5 relative z-10">
                          ...
                          <div className="flex items-center gap-2 text-[9px] font-mono text-emerald-500/70 font-bold">
                            <div className="w-1 h-1 rounded-full bg-emerald-500/30" />
                            {device.ipv4}
                          </div>
                        </div>
